...
def main():
    ...
    st = ase.get_structure(read(filename))
    filename=filename.split('/')[-1].split('.')[0]
    print(filename)
    if typecalc == "single":
        json_dir = "data"
        json_fn = Path(f"{json_dir}/mp_structures/{filename}/{filename}.json") 
...
#        printKgrid( st, folder )
        ...
        koffset = [0.0, 0.0, 0.0]
        params['scf.kpoints'] = kpoints
        makeExcitingXAS( filename, st, params )
        folder = Path(f"{json_dir}/mp_structures/{filename}/EXCITING")
        print(folder)
        printKgrid( st, folder )
        ...
        params['scf.kpoints'] = kpoints
        makeXspectra( filename, st, params )
        folder = Path(f"{json_dir}/mp_structures/{filename}/XS")
        printKgrid( st, folder )
 ...
 #           makeOcean( mpid, st, params )
        ...
        folder = Path(f"{json_dir}/mp_structures/{filename}/OCEAN")
        printKgrid( st, folder )
        folder = Path(f"{json_dir}/mp_structures/{filename}/EXCITING")
        printKgrid( st, folder )
