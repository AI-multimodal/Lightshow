...
for element in inJSON:
    ...
    m = re.search('z_valence="\s*(\d+\.?\d*([Ed][+-]?\d+)?)\s*"', pspString.decode(), re.IGNORECASE )
    if m:
        print( int(float(m.group(1))), ' :    ', m.group(0))
        zval = int(float(m.group(1)))
    else:
        m = re.search('\s*(\d+\.?\d*)\s+z valence', pspString.decode(), re.IGNORECASE )
        print( int(float(m.group(1))), ' :    ', m.group(0))
...
with open( "zval.json", 'w' ) as f:
    f.write( json.dumps( inJSON, sort_keys=True, indent=3, separators=(',', ': '))  )
