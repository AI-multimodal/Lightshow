...
from xanes_bench.CurveComparisons.compare_pasers import (
    XSplot,
    OCEANplot,
    Excitingplot,
)
...
def calculate_similarity_last(mpid, code):
    ...
    last_spectra_fn = (
        Path(mpid) / code / list(spectra_folder_dict.keys())[num_folders - 1]
    )
    if code == "XS":
    ...
    while not last_spectra.exists():
        ...
        last_spectra_fn = (
            Path(mpid)
            / code
            / list(spectra_folder_dict.keys())[num_folders - 1]
        )
        if code == "XS":
            last_spectra = XSplot(last_spectra_fn, ["0"])  # site_list)
        elif code == "OCEAN":
    ...
    for i in spectra_folder_dict:
        ...
        if this_spectra.exists():
            spearman = comparePlots(
                0, this_spectra.spectra, last_spectra.spectra
            )[1]
...
def calculate_similarity_next(mpid, code):
    ...
    for i in range(len(spectra_folder_list) - 1):
        ...
        if code == "XS":
            ...
            second_spectra = Excitingplot(
                second_spectra_fn, ["0"]
            )  # site_list)

        spearman = comparePlots(
            0, first_spectra.spectra, second_spectra.spectra
        )[1]
        out[spectra_folder_dict[spectra_folder_list[i]]] = np.log10(
            1 - spearman
        )
    return out
