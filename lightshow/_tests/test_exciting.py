...
def test_default_parameters(mp_Structure_mp390: Structure, tmp_path: Path):
    ...
    write_results = exciting_parameters.write(tmp_path, structure_uc=mp_Structure_mp390, sites=[1])
    ...
    assert len(list(structure_xml)) == 3, 'Expect structure tree to have 3 sub-elements'
    ...
    assert lattice_vectors[0].text == "     -1.90135500       1.90135500       4.87387600", \
        "Lattice vector `a` differs from input"
    assert lattice_vectors[1].text == "      1.90135500      -1.90135500       4.87387600", \
        "Lattice vector `b` differs from input"
    assert lattice_vectors[2].text == "      1.90135500       1.90135500      -4.87387600", \
    ...
    assert species_ti_xml.items() == [('speciesfile', 'Ti.xml')], 'species is inconsistent'
    ...
    assert atoms_h[0].items() == [('coord', '      0.75000000       0.25000000       0.50000000')], \
        "Coordinate of first Ti differs to input"
    assert atoms_h[1].items() == [('coord', '      0.00000000       0.00000000       0.00000000')], \
        "Coordinate of second Ti differs to input"

    atoms_he = list(species_o_xml)
    assert len(atoms_he) == 4, 'Number of O atoms is wrong'
    assert atoms_he[0].items() == [('coord', '      0.95616300       0.45616300       0.50000000')], \
        "Coordinate of first O differs to input"
    assert atoms_he[1].items() == [('coord', '      0.79383700       0.79383700       0.00000000')], \
        "Coordinate of second O differs to input"
    assert atoms_he[2].items() == [('coord', '      0.54383700       0.04383700       0.50000000')], \
        "Coordinate of third O differs to input"
    assert atoms_he[3].items() == [('coord', '      0.20616300       0.20616300       0.00000000')], \
        "Coordinate of fourth O differs to input"

    groundstate_xml = subelements[2]
    assert groundstate_xml.tag == 'groundstate'
    assert len(groundstate_xml.keys()) == 9, 'Excpect groundstate to have 9 attributes.'
    assert set(groundstate_xml.keys()) == \
           {'rgkmax', 'do', 'ngridk', 'xctype', 'nempty', 'gmaxvr', 'lmaxmat', 'lmaxvr', 'lmaxapw'}, \
    ...
    assert set(xs_xml.keys()) == \
           {'broad', 'ngridk', 'xstype', 'nempty', 'gqmax', 'vkloff', 'tevout', 'tappinfo', 'ngridq'}, \
    ...
    assert energywindow_xml.tag == "energywindow", 'First xs subelement should be energywindow'
    ...
    assert screening_xml.tag == "screening", 'Second xs subelement should be screening'
    ...
    assert set(bse_xml.keys()) == \
           {'bsetype', 'xas', 'xasedge', 'nstlxas', 'distribute', 'eecs', 'xasspecies', 'xasatom'}
    ...
    assert qpointset_xml.tag == "qpointset", 'Fifth xs subelement should be qpointset'
...
def test_exciting_own_parameters(tmp_path: Path, mp_Structure_mp390: Structure):
    ...
    exciting_parameters = EXCITINGParameters(own_cards, 'keks', ['piep', 'blub'], name='Test')
    assert exciting_parameters.name == 'Test'

    write_results = exciting_parameters.write(tmp_path, structure_uc=mp_Structure_mp390, sites=[1, 2, 3])
    assert write_results == {'pass': True, 'errors': {}}
    ...
    structure_xml = subelements[1]
    assert structure_xml.tag == 'structure'
    assert structure_xml.get('speciespath') == 'keks', 'species path set to keks'

    groundstate_xml = subelements[2]
    assert groundstate_xml.tag == 'groundstate'
    assert len(groundstate_xml.keys()) == 2, 'Excpect groundstate to have 2 attributes.'
    assert set(groundstate_xml.keys()) == {'ngridk', 'nempty'}, 'Excpected groundstate keys differ from found keys.'
    assert groundstate_xml.get('ngridk') == "5 5 6"
    ...
    assert set(xs_xml.keys()) == {'ngridk', 'nempty', 'ngridq', 'xstype', 'gqmax'}, 'Excpected xs keys differ from found keys.'
    assert xs_xml.get('gqmax') == '4.0'
    assert xs_xml.get('ngridk') == '5 5 6'
    assert xs_xml.get('xstype') == 'BSE'
