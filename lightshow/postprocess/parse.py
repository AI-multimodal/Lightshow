...
def extract_FEFF(path): 
    ...
    dict_output
        ...
        ``{"label": "...", "energy": [...], "spectrum": [...], "efermi": ..., "normalization_constant": ...}``
    """
    
    dict_output = {} 
    dict_output['label'] = 'cross-section'  # FEFF outputs the X-ray absorption cross-section. This label may be used for later analysis. 
    path = Path(path)
    
    # Extract sepctrum and normalization constant
    # The normalization constant can be used to normalize feff to absolute cross section in the unit of Angstrom^2 
    with open(path/"xmu.dat", 'r') as f:
        spectrum = np.loadtxt(f.readlines())
        dict_output['energy'] = spectrum[:,0]
        dict_output['spectrum'] = spectrum[:,3]
        f.seek(0)
        dict_output['normalization_constant'] = [float(line.split()[-1]) for line in f.readlines() if 'normalize mu' in line][0]
...
def extract_VASP(path): 
    ...
    dict_output
        ...
        ``{"label": ..., "energy": [...], "efermi": ..., "xx", [...], "yy", [...], "zz", [...], "spectrum": [...], "volume": ..., "total_energy": ...}``
    """
    
    dict_output = {}
    dict_output['label'] = 'epsilon2'  # VASP outputs the imaginary part of the dielectric tensor. This label may be used for later analysis. 
    ...
    dict_output['xx'] = mu[:,1] # xx, yy, zz are the three polarization directions
    dict_output['yy'] = mu[:,2]
    dict_output['zz'] = mu[:,3]
...
def extract_XSpectra(path, es_out_file=None): 
    ...
    dict_output
        ...
        Fermi level (in eV) and Total energy (in eV) are extracted if the path and file name of es.out file is given. 
        ``{"label": ..., "energy": [...], "spectrum": [...], "dipole1", [...], "dipole2", [...], "dipole3", [...], "efermi": ..., "total_energy": ...}``
    ...
    dict_output = {}
    dict_output['label'] = 'cross-section'  # XSpectra outputs the X-ray absorption cross-section. This label may be used for later analysis. 
    path = Path(path)
    ...
    if num_polar > 3: 
        warnings.warn("More than three \'xanes.dat\' files in %s. Might be a problem."%path)
    ...
    if es_out_file is not None: 
        with open(es_out_file, 'r') as f:
            dict_output['total_energy'] = [float(line.split()[-2]) * physical_constants['Rydberg constant times hc in eV'][0] for line in f.readlines() if '!' in line][0]
...
def extract_OCEAN(path, scf_out_file=None): 
    ...
    dict_output
        ...
        ``{
            "Ti": {
                    "0001_1s": {"label": ..., "energy": [...], "spectrum": [...], "01": [...], "02": [...], "03": [...], "efermi": ..., "total_energy": ...}, 
                    "0002_1s": {...},
                    ...
                    }, 
           }``
        The sub-dictionary of one site is dict_output["Ti"]["0001_1s"], which has the same structure and keys as the output of the parsing functions for other codes. 
    ...
    if scf_out_file is not None: 
        with open(scf_out_file, 'r') as f:
            for line in f.readlines():
                ...
                if '!' in line:
                    total_energy = float(line.split()[-2]) * physical_constants['Rydberg constant times hc in eV'][0]
    ...
    for element in dict_output.keys(): 
        for absorber in dict_output[element].keys():
            dict_output[element][absorber]['spectrum'] /= dict_output[element][absorber]['num_polar']
...
def extract_exciting(path, INFO_out_file=None): 
    ...
    dict_output
        ...
        ``{"label": ..., "energy": [...], "spectrum": [...], "11": [...], "22": [...], "33": [...], "efermi": ..., "total_energy": ...}``
    ...
    dict_output = {}
    dict_output['label'] = 'epsilon2'  # EXCITING outputs the imaginary part of the dielectric tensor. This label may be used for later analysis. 
    path = Path(path)
    ...
    # Print a reminder if there are more than three polarizations in the directory. 
    if num_polar > 3:
        warnings.warn("More than three \'EPSILON...OUT\' files in %s. Might be a problem."%path)
    dict_output['spectrum'] = spectra/num_polar
        ...
        with open(INFO_out_file, 'r') as f:
            for line in f.readlines():
                ...
                if 'Total energy' in line:
                    total_energy = float(line.split()[3])*physical_constants['Hartree energy in eV'][0]
            dict_output['efermi'] = float(lines_fermi[-2].split()[3])*physical_constants['Hartree energy in eV'][0]
            dict_output['total_energy'] = total_energy
